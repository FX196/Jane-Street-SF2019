...
def trade(exchange):
    ...
    if diff > 10:
        size = 1
        trades.append(("BUY", "VALBZ", int(exchange.trade_prices["VALBZ"]), size))
        trades.append(("CONVERT", "BUY", "VALE", size))
        trades.append(("SELL", "VALE", int(exchange.trade_prices["VALE"]), size + exchange.holdings["VALE"]))
    elif diff < 10:
        size = 1
        trades.append(("SELL", "VALBZ", int(exchange.trade_prices["VALBZ"]), size + exchange.holdings["VALBZ"]))
        trades.append(("CONVERT", "SELL", "VALE", size))
        trades.append(("BUY", "VALE", int(exchange.trade_prices["VALE"]), size))
    return trades
